...
macro_rules! declare_terminal {
    ...
    ( -: $lexer_name:ident :-  $($name:ident),* ) => {
        ...
        let preprocor
        = $crate::parser::CommentPreProcessor::new(
            "comment preprocor",
            comment_m_vec
        );

        let $lexer_name
        = $crate::parser::Lexer::new(
            "generated by code",
            matcher_vec,
            vec![
                Box::new(preprocor)
...
mod test {
    ...
    fn test_comment_preprocor() {
        ...
        let res_r_s: String = replace_with_samelenspace(&test_script1_chars, (l, r)).iter().collect();
        println!("{}", res_r_s);
    }
}
